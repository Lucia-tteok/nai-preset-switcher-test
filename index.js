...
!async function() {
    ...
    function B() {
        ...
        function fillSettingsParams(t) {
            [{
                id: "#nl-set-nai-sampler",
                key: "naiSampler"
            }, {
                id: "#nl-set-nai-steps",
                key: "naiSteps"
            }, {
                id: "#nl-set-nai-guidance",
                key: "naiPromptGuidance"
            }, {
                id: "#nl-set-nai-rescale",
                key: "naiPromptGuidanceRescale"
            }].forEach(function(n) {
                var r = e.querySelector(n.id);
                r && (r.value = t[n.key] || "")
            })
        }
        var settingsRefresh = e.querySelector("#nl-set-nai-refresh"),
            settingsApply = e.querySelector("#nl-set-nai-apply");
        settingsRefresh && settingsRefresh.addEventListener("click", function() {
            fillSettingsParams(nlGetChatuNaiParams()), E("已读取智绘姬当前参数", "success")
        });
        settingsApply && settingsApply.addEventListener("click", function() {
            nlApplyChatuNaiParams(settingsParamsFromInputs()) ? E("已写入智绘姬参数", "success") : E("未检测到可写入的智绘姬参数控件", "warning")
        });
