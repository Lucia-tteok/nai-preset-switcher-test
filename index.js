...
!async function() {
    ...
    function nlShowImagePreview(e, t) {
        ...
        a && a.remove();
        a = s.createElement("div"), a.className = "nl-img-preview", a.setAttribute("style", "position:absolute;inset:0;z-index:80;display:flex;align-items:center;justify-content:center;background:rgba(20,24,30,.62);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);padding:18px;"), a.innerHTML = '<div style="max-width:min(92vw,520px);max-height:82vh;display:flex;flex-direction:column;gap:10px;align-items:center;"><img src="' + e + '" style="max-width:100%;max-height:72vh;border-radius:14px;object-fit:contain;box-shadow:0 18px 60px rgba(0,0,0,.32);"><div style="color:#fff;font-size:13px;text-shadow:0 1px 4px rgba(0,0,0,.45);">' + k(t || "单击空白处关闭") + "</div></div>", a.addEventListener("click", function(e) {
            e.target === a && a.remove()
    ...
    async function nlRefreshFloatingBall() {
        ...
        e || (e = s.createElement("div"), e.id = "nl-floating-ball-v2", e.setAttribute("style", "position:fixed;right:18px;bottom:82px;width:58px;height:58px;border-radius:50%;z-index:99999;overflow:hidden;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,.86);border:1px solid rgba(124,108,255,.45);box-shadow:0 10px 28px rgba(40,55,70,.26);cursor:pointer;backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);"), s.body.appendChild(e));
        var t = nlCurrentPresetName(), n = null;
        try {
            var r = await I.all();
            n = r.filter(function(e) {
                return (e.name || "").trim() === (t || "").trim()
            })[0] || null
        } catch (e) {}
        e.title = t ? "当前预设：" + t : "当前预设";
        e.innerHTML = n && n.thumb ? '<img src="' + n.thumb + '" style="width:100%;height:100%;object-fit:cover;display:block;">' : '<div style="font-size:24px;color:#8a97a4;line-height:1;">&#128247;</div>';
        e.onclick = function() {
            n && n.thumb ? nlShowImagePreview(n.thumb, t || "当前预设") : E("当前预设暂无预览图", "info")
        }
    }
    ...
    function B() {
        ...
        ["#nl-set-nai-sampler", "#nl-set-nai-steps", "#nl-set-nai-guidance", "#nl-set-nai-rescale"].forEach(function(t) {
            var n = e.querySelector(t);
            n && ["input", "change"].forEach(function(t) {
                n.addEventListener(t, function() {
                    nlApplyChatuNaiParams(settingsParamsFromInputs())
                })
            })
        });
        ...
        b && b.addEventListener("click", async () => {
            ...
            const vibeBinding = getCurrentVibeBinding(),
                naiParams = nlGetSettingsNaiParams(e),
                r = {
                    id: n ? n.id : S(),
                    name: t,
                    category: d.slice(),
                    ...
                    naiSampler: naiParams.naiSampler,
                    naiSteps: naiParams.naiSteps,
                    naiPromptGuidance: naiParams.naiPromptGuidance,
                    naiPromptGuidanceRescale: naiParams.naiPromptGuidanceRescale
                };
            try {
                await I.put(r);
                var a = !1;
                try {
                    W() && (Y(t, r.positive, r.negative), nlApplyChatuNaiParams(r), a = !0)
                } catch (e) {}
    ...
    async function R() {
        ...
        A && A.addEventListener("input", e => {
            ...
            var thumbEl = e.querySelector(".nl-thumb");
            thumbEl && thumbEl.addEventListener("click", function(t) {
                t.stopPropagation();
                const n = e.getAttribute("data-id");
                if (thumbEl._nlThumbClickT) {
                    clearTimeout(thumbEl._nlThumbClickT);
                    thumbEl._nlThumbClickT = null;
                    return
                }
                thumbEl._nlThumbClickT = setTimeout(async function() {
                    thumbEl._nlThumbClickT = null;
                    try {
                        var e = await I.get(n);
                        e && e.thumb ? nlShowImagePreview(e.thumb, e.name) : E("该预设暂无预览图", "info")
                    } catch (e) {}
                }, 240)
            });
            thumbEl && thumbEl.addEventListener("dblclick", async function(t) {
                ...
                try {
                    var a = await I.get(n);
                    a && nlChangePresetThumb(a, null, function() {
                        nlRefreshFloatingBall(), R()
                    })
                } catch (e) {}
            });
            e.addEventListener("click", () => {
    ...
    async function Q(e) {
        ...
        if ($) {
            $.addEventListener("click", function(e) {
                e.stopPropagation(), n.thumb ? nlShowImagePreview(n.thumb, n.name) : E("该预设暂无预览图", "info")
            });
            $.addEventListener("dblclick", function(e) {
                e.stopPropagation(), e.preventDefault(), nlChangePresetThumb(n, i, function() {
