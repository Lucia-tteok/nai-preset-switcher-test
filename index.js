...
!async function() {
    ...
    function nlShowImagePreview(e, t) {
        ...
        a && a.remove();
        a = s.createElement("div"), a.className = "nl-img-preview", a.setAttribute("style", "position:absolute;inset:0;z-index:10004;display:flex;align-items:center;justify-content:center;background:rgba(20,24,30,.62);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);padding:18px;"), a.innerHTML = '<div style="max-width:min(92vw,520px);max-height:calc(var(--nl-app-h,100vh) * 0.82);display:flex;flex-direction:column;gap:10px;align-items:center;"><img src="' + e + '" style="max-width:100%;max-height:calc(var(--nl-app-h,100vh) * 0.72);border-radius:14px;object-fit:contain;box-shadow:0 18px 60px rgba(0,0,0,.32);"><div style="color:#fff;font-size:13px;text-shadow:0 1px 4px rgba(0,0,0,.45);">' + k(t || "单击空白处关闭") + "</div></div>", a.addEventListener("click", function(e) {
            e.target === a && a.remove()
    ...
    function nlBindViewportSync() {
        nlSyncViewportVars();
        if (window.__naiLibViewportSyncBound) return;
        window.__naiLibViewportSyncBound = true;
        var run = function() {
            nlSyncViewportVars();
            try { requestAnimationFrame(nlSyncViewportVars); } catch (_) {}
        };
        try { window.addEventListener("resize", run, { passive: true }); } catch (_) { window.addEventListener("resize", run); }
        try { window.addEventListener("orientationchange", run); } catch (_) {}
        try {
    ...
    function nlOpenPresetSwitcherPanel() {
        var e = N();
        e && (nlBindViewportSync(), e.style.display = "flex", nlSetFloatingBallPanelOpen(!0), M("lib"))
    }
...
function bindFields(){
    ...
    function ce() {
        try {
            ...
            var a = function(e) {
                e && (e.preventDefault(), e.stopPropagation()); var panel = N(); nlBindViewportSync(); panel.style.display = "flex"; var active = panel.querySelector(".nl-tab.active"); active && active.getAttribute("data-tab") === "lib" ? R() : M("lib")
            };
